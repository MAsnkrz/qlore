"""
Qogita L'Oréal Monitor — Catalog Download API version
Monitors https://www.qogita.com/brands/loreal/

Uses Qogita's official public Buyer API endpoint:
  GET /variants/search/download/?brand_name=L'Oréal
...
BRAND_NAME       = "L'Oréal"          # confirmed exact match for the catalog API
BRAND_NAME_FALLBACKS = ["L'Oreal", "LOreal", "Loreal", "L Oreal"]
BRAND_PAGE_URL   = "https://www.qogita.com/brands/loreal/"
SNAPSHOT_FILE    = "snapshot_qogita_loreal.json"
BASELINE_FLAG    = "baseline_done_loreal.txt"
...
def notify_new(product):
    ...
    embed = {
        ...
        "footer":    {"text": "Qogita L'Oréal Monitor • qogita.com"},
    }
...
def run_check():
    print(f"\n[{datetime.now(timezone.utc).strftime('%H:%M:%S UTC')}] Checking Qogita L'Oréal...")
...
def main():
    print("=" * 55)
    print("  Qogita L'Oréal Monitor (Catalog Download API)")
